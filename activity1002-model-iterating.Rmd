...
```{r}
evals %>%
  summarise(min= min(score), q1=quantile(score, probs=.25), mean=mean(score), median=median(score), q3=quantile(score, probs=.75), max=max(score))
...
**Response**:
The data appears to be left skewed with a few really bad classes pulling the mean down. this doesn't suprise me too much as I'd expect most classes to be good with the exception of a few bad ones.
...
```{r}
evals %>%
  summarise(min= min(bty_avg), q1=quantile(bty_avg, probs=.25), mean=mean(bty_avg), median=median(bty_avg), q3=quantile(bty_avg, probs=.75), max=max(bty_avg))
evals %>%
...
**Response**:
This is fairly evenly distributed which doesn't surpise me all that much. There appears to be very little correlation between the score of the class and the attractiviness of the teacher.
...
```{r}
evals %>%
  ggplot(mapping=aes(x=bty_avg,y=score, color=gender)) +
  geom_jitter()
```
...
**Response**:
There still doesn't appear to be any noticable trends here.
...
3.74733459 + 0.07415493(bty_avg) + 0.17238871(gendermale)
...
There does appear to be a slight positive correlation between the 2 variables. However, glancing at the summary model statistics, they appear to account for only about 6% of variation in class score which is very low overall.
...
```{r}
nested_data %>% 
  mutate(model = map(data, ~lm(score ~ bty_avg + gender, data=evals)), tidied= broom::tidy(model))
```
